...
class Bazel(FlagsSource):
    ...
    def __init__(self,
        ...
        self.__generation_script = generation_script
    ...
    def get_flags(self, file_path=None, search_scope=None):
        ...
        log.debug("[bazel]:[get]: for file %s", file_path)
        cached_bazel_path = self._get_cached_from(file_path)
        log.debug("[bazel]:[cached]: '%s'", cached_bazel_path)
        current_bazel_file = File.search(
            file_name=self._FILE_NAME,
            search_scope=search_scope)
        if not current_bazel_file:
            log.debug("No valid '%s' file found.", self._FILE_NAME)
            return None
        current_bazel_path = current_bazel_file.full_path
        log.debug("[bazel]:[current]: '%s'", current_bazel_path)

        parsed_before = current_bazel_path in self._cache
        if parsed_before:
            log.debug("[bazel]: found cached WORKSPACE.")
            cached_bazel_path = current_bazel_path
            # remember that for this file we have found this cmakelists
            self._cache[file_path] = current_bazel_path
        path_unchanged = (current_bazel_path == cached_bazel_path)
        file_unchanged = File.is_unchanged(cached_bazel_path)
        if path_unchanged and file_unchanged:
            use_cached = True
            if Bazel.__need_cmake_rerun(cached_bazel_path):
                use_cached = False
            if cached_bazel_path not in self._cache:
                use_cached = False
            if use_cached:
                log.debug("[bazel]:[unchanged]: use existing db.")
                db_file_path = self._cache[cached_bazel_path]
                db = CompilationDb(
        ...
        log.debug("[bazel]:[generate new db]")
        db_file = Bazel.__compile_cmake(
            cmake_file=File(current_bazel_path),
        ...
        if file_path:
            ...
            self._cache[file_path] = current_bazel_path
            self._cache[current_bazel_path] = db_file.full_path
            File.update_mod_time(current_bazel_path)
        db = CompilationDb(
            self._include_prefixes,
            self.__header_to_source_mapping,
            self.__lazy_flag_parsing)
    ...
    def __compile_cmake(cmake_file, cmake_binary, prefix_paths, flags,
        ...
        tempdir = Bazel.unique_folder_name(cmake_file.full_path)
        try:
            os.makedirs(tempdir)
        except OSError:
        ...
        if path.exists(dep_file_path):
            for dep_path in Bazel.__get_cmake_deps(dep_file_path):
    ...
    def __get_cmake_deps(deps_file):
        ...
        with open(deps_file, 'r') as f:
            content = f.read()
            found = Bazel._DEP_REGEX.findall(content)
            for dep in found:
    ...
    def __need_cmake_rerun(cmake_path):
        tempdir = Bazel.unique_folder_name(cmake_path)
        ...
        for dep_file in Bazel.__get_cmake_deps(dep_file_path):
